...
import { Wallet, Mail, Music, FileText, HardDrive, Calendar, Search, Table, Share2, Briefcase, Store, Wifi, Volume2, Battery, Clock, TrendingUp, Building2, Shield, Video, Code2, Camera, MapPin, MessageCircle, Users, Gamepad2, BookOpen, Globe, Box, Monitor, GraduationCap, Paintbrush, UserCheck, Minimize2, Home, Sparkles } from 'lucide-react';
import './Dock.css';
...
const Dock: React.FC = () => {
  ...
  const dockApps: DockApp[] = [
    ...
    { name: 'Bitcoin Music', icon: Music, color: 'rainbow', url: 'https://bitcoin-music.vercel.app', current: true },
      ...
      <div className="dock-container">
            ...
            return (
              ...
              >
                {app.id === 'bapps-store' ? (
                  ...
                  <svg className="dock-app-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} style={{ color: getIconColor(app.color, index) }}>
        ...
        <div className="dock-status">
          ...
          <button 
            className="status-button" 
            title="BAPPS"
            onClick={() => window.location.href = 'https://www.bitcoinapps.store/'}
          >
          ...
          <button 
            className="status-button" 
            title="CashBoard"
            onClick={() => window.location.href = 'https://www.cashboard.website/'}
          >
            <svg className="status-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} style={{ color: '#ffffff' }}>
              <circle cx="12" cy="12" r="10" />
              <path d="M15.5 9.5C14.815 8.574 13.743 8 12.5 8c-2.21 0-4 1.79-4 4s1.79 4 4 4c1.243 0 2.315-.574 3-1.5" />
            </svg>
          </button>
          <button 
            className="status-button" 
            title="Senseii"
            onClick={() => window.location.href = 'https://senseii-zeta.vercel.app/'}
          >
            <svg className="status-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} style={{ color: '#ef4444' }}>
              <path d="M3 5h18" />
              <path d="M6 5v14" />
              <path d="M18 5v14" />
              <path d="M3 9h18" />
